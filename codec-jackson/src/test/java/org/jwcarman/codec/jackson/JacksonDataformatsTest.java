...
/**
 * {@link JacksonCodecFactory} takes any {@link ObjectMapper}, so every Jackson dataformat is a
 * backend by mapper swap. These tests prove that claim for the binary and text formats Jackson
 * itself ships.
 */
@DisplayNameGeneration(DisplayNameGenerator.ReplaceUnderscores.class)
class JacksonDataformatsTest {
  ...
  class Every_dataformat {
    ...
    @MethodSource("org.jwcarman.codec.jackson.JacksonDataformatsTest#mappers")
    void round_trips_a_record(String format, ObjectMapper mapper) {
      Codec<Person> codec = new JacksonCodecFactory(mapper).create(Person.class);
    ...
    void round_trips_a_generic_map(String format, ObjectMapper mapper) {
      Codec<Map<String, Person>> codec =
          new JacksonCodecFactory(mapper).create(new TypeRef<Map<String, Person>>() {});
    ...
    void output_is_not_json(String format, ObjectMapper mapper) {
      byte[] encoded = new JacksonCodecFactory(mapper).create(Person.class).encode(ALICE);
  ...
  class Binary_formats {
    ...
    void cbor_and_smile_round_trip_a_generic_list() {
      for (ObjectMapper mapper :
        ...
        Codec<List<Person>> codec =
            new JacksonCodecFactory(mapper).create(new TypeRef<List<Person>>() {});
    ...
    void smile_output_carries_its_header() {
      byte[] encoded =
          new JacksonCodecFactory(SmileMapper.builder().build()).create(Person.class).encode(ALICE);
  ...
  class Text_formats {
    ...
    void yaml_output_is_readable_yaml() {
      byte[] encoded =
          new JacksonCodecFactory(YAMLMapper.builder().build()).create(Person.class).encode(ALICE);
    ...
    void xml_output_is_an_element_named_after_the_type() {
      byte[] encoded =
          new JacksonCodecFactory(XmlMapper.builder().build()).create(Person.class).encode(ALICE);
